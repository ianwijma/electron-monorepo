...
export const normalizeFilePath = (relativePath: string): string => {
...
export const denormalizeFilePath = (absolutePath: string): string => {
...
export const ensureSafeFilePath = (absolutePath: string): void => {
    if (!absolutePath.startsWith(ROOT_DIR)) {
        throw new Error(
            `Path "${absolutePath}" tried to get outside our own directory, this is NOT allowed.`,
        );
    }
};
...
export const readStringFile = async (
    relativeFilePath: string,
): Promise<string> => {
    const normalizePath = normalizeFilePath(relativeFilePath);
...
export const copyFile = async (
    fromRelativeFilePath: string,
    toRelativeFilePath: string,
): Promise<void> => {
 ...
 */
export const UNSAFE_fileExists = async (
    absolutePath: string,
): Promise<boolean> => {
...
export const UNSAFE_fileStats = async (
    absolutePath: string,
): Promise<Stats | false> => {
    return await fs.stat(absolutePath).catch(() => false);
};

export const fileStats = async (
    relativeFilePath: string,
): Promise<Stats | false> => {
    const normalizePath = normalizeFilePath(relativeFilePath);
...
export const ensureYamlFileExists = async <T>(
    relativeFilePath: string,
    defaultContent: T,
): Promise<string> => {
...
export const ensureJsonFileExists = async <T>(
    relativeFilePath: string,
    defaultContent: T,
): Promise<string> => {
