'use client';

import {BaseContainer} from 'frontend-essentials/src/components/windows/containers/baseContainer'
import {BaseTitlebar} from 'frontend-essentials/src/components/windows/titlebars/baseTitlebar'
import {KeyboardShortcuts} from 'frontend-essentials/src/components/keyboard-shortcuts'
import {KeyboardSettings} from "qlippy-common/src/settings/keyboard.settings.types";
import {useSettings} from "frontend-essentials/src/hooks/useSettings";

// TODO: replace
// @ts-ignore
export const logo = 'data:image/png;base64,';

export default function SettingsPage() {
    const {isLoading, settings, updateSettings} = useSettings<KeyboardSettings>('keyboard');

    if (isLoading) return 'loading...'

    const {shortcuts} = settings;

    const activateShortcut = Object.values(shortcuts).filter(({target}) => target === 'window')[0] ?? undefined;

    if (!activateShortcut) {
        return <div>Unable to find activate shortcut</div>
    }

    const onActivatorChanged = (activator: string) => {
        const {[activateShortcut.shortcut]: _, ...otherShortcut} = shortcuts;

        updateSettings({
            ...settings,
            shortcuts: {
                ...otherShortcut,
                [activator]: {
                    ...activateShortcut,
                    shortcut: activator
                }
            }
        })
    }

    return <BaseContainer
        title='Qlippy Settings'
        color='#3c3f41'
        borderColor='#2b2b2b'
        titleBar={<BaseTitlebar title='Qlippy Settings' logo={logo} color='#2b2b2b'/>}>
        {/* TODO: Update by better way to represent settings */}
        <div className='flex'>
            <label className='w-1/3'>
                Activation
            </label>
            <KeyboardShortcuts onShortcutChanged={onActivatorChanged} accelerate={activateShortcut.shortcut}/>
        </div>
    </BaseContainer>;
}